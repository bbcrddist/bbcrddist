- Unregister any registered cleanup functions.
- To manually invoke the cleanup callbacks (without exiting):
- If the running go program receives a SIGINT, or SIGTERM cleanup will intercept the signal, run all the registered clean up functions and then exit with status 1.
